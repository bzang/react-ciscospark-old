...
export class MessageWidget extends Component {
  ...
  listenToTypingEvents(conversationId, spark) {
    ...
    spark.mercury.on(`event:status.start_typing`, handleStartTyping);

    function handleStartTyping(event) {
      if (event.data.conversationId === conversationId) {
        props.setTyping(event.data.actor.id, true);
      }
    }

    spark.mercury.on(`event:status.stop_typing`, handleStopTyping);
  ...
  listenToNewActivity(conversationId, spark) {
    const {handleEvent, props} = this;
    spark.mercury.on(`event:conversation.activity`, handleActivity);
